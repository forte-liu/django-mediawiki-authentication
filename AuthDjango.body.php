<?php
    ...
    class AuthDjango extends AuthPlugin {
        ...
        public function __construct() {
            ...
            // start Django database connection
    	    $DB_HOST = $GLOBALS['wgAuthDjangoConfig']['DjangoHost'];
    	    $DB_PORT = $GLOBALS['wgAuthDjangoConfig']['DjangoPort'];
    	    $DB_NAME = $GLOBALS['wgAuthDjangoConfig']['DjangoDBName'];
    	    $DB_USER = $GLOBALS['wgAuthDjangoConfig']['DjangoUser'];
    	    $DB_PASS = $GLOBALS['wgAuthDjangoConfig']['DjangoPass'];

    	    $conn_string = "host=$DB_HOST dbname=$DB_NAME user=$DB_USER password=$DB_PASS";
    	    $this->dbd = pg_connect($conn_string);

    	    if(!$this->dbd) {
                error_log("Error: Unable to open django database\n");
    	    }
        ...
        public function onUserLoadFromSession($user, &$result) {
            ...
            $pageTitle = SpecialPageFactory::getLocalNameFor('Userlogin');

            if (isset($_REQUEST['title']) && strstr($_REQUEST['title'], $pageTitle)) {
                ...
                $result = pg_query_params($this->dbd, "SELECT * FROM ".$this->session_profile_table." WHERE session_id=$1", array($django_session));
                $r1 = pg_fetch_object($result);

                if ($r1) {
                    if ($r1->user_id) {
                        $user_id = $r1->user_id;
                    	$result = pg_query_params($this->dbd, "SELECT username, email FROM ".$this->user_table." WHERE id=$1", array($user_id));
                    	$r1 = pg_fetch_object($result);
                    } else {
                        $r1 = false;
                    }
